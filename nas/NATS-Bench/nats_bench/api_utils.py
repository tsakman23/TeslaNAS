...
class NASBenchMetaAPI(metaclass=abc.ABCMeta):
    ...
    def simulate_train_eval(
        ...
        all_names = ("cifar10", "cifar100", "ImageNet16-120", "synthetic-cifar10", "Tiny10", "synthetic-Tiny10")
        ...
        if "valid-accuracy" in info:
            ...
            try:
                ...
                time_cost = info["train-all-time"] + temp_info["valtest-per-time"]
...
class ArchResults(object):
    ...
    def get_metrics(self, dataset, setname, iepoch=None, is_random=False):
        ...
        for result in results:
            if not isinstance(result, ResultsCount):
                raise ValueError(
                    "invalid result type : {:}".format(type(result))
                )
            if setname == "train":
