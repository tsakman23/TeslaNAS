...
##############################################################################
import re, time, shutil, random, argparse
import numpy as np
import torch
...
from nats_bench import pickle_save, ArchResults, ResultsCount
...
def create_result_count(
    ...
    if "train_times" in results:  # new version
        ...
        if dataset == "cifar10-valid":
            ...
            xresult.update_latency(latencies)
        elif dataset == "synthetic-cifar10": # doesn't even matter
            xresult.update_OLD_eval(
                "ori-test", results["valid_acc1es"], results["valid_losses"]
            )
            loss, top1, top5, latencies = pure_evaluate(
            ...
            loss, top1, top5, latencies = pure_evaluate(
                dataloader_dict["{:}@{:}".format(dataset, "valid")], network.cuda() # using the valid set
            )
            xresult.update_latency(latencies)
        elif dataset == "synthetic-Tiny10":
            xresult.update_OLD_eval(
                "ori-test", results["valid_acc1es"], results["valid_losses"]
            )
            loss, top1, top5, latencies = pure_evaluate(
    ...
    for checkpoint_path in checkpoints:
        ...
        for dataset in datasets:
            if dataset not in checkpoint:
                ...
                if args.v:
                    print(f"Found {dataset} in arch-{arch_index} from {checkpoint_path}")
                ok_dataset += 1
            ...
            arch_config = {
                "channel": results["arch_config"]["C"],
                "num_cells": results["arch_config"]["N"],
                "arch_str": results["arch_config"]["genotype"],
...
def correct_time_related_info(arch_index: int, arch_infos: Dict[Text, ArchResults]):
    ...
    nums = {
        ...
        "cifar100-test": 10000,
        "cifar100-valid": 5000,
        "synthetic-cifar10-train": 1500,
        "synthetic-cifar10-test": 10000,
        "Tiny10-train": 5000,
        "Tiny10-test": 500, 
        "synthetic-Tiny10-train": 1500,
        "synthetic-Tiny10-test": 500,
    }
...
def simplify(save_dir, save_name, nets, total, sup_config):
    dataloader_dict = get_nas_bench_loaders(6, args.dataset)
    hps, seeds = ["12", "200"], set()
    for hp in hps:
        sub_save_dir = save_dir / "raw-data-{:}".format(hp)
        # CHECK IF EXISTS
        if not sub_save_dir.exists():
            if args.v:
                print(f"Oh no!, the directory {sub_save_dir} does not exist.")
            continue
        ...
        if args.v:
            print(f"DEBUG : {max(nums)=}"),
            print(f"DEBUG : {total=}"),
            print(f"DEBUG : {len(nets)=}"),
        # assert (            
        #     len(nets) == total == max(nums)
        # ), "there are some missed files : {:} vs {:}".format(max(nums), total)
    print("{:} Start simplifying the checkpoint.".format(time_string()))

    datasets = ("cifar10-valid", "cifar10", "cifar100", "ImageNet16-120", "synthetic-cifar10", "Tiny10", "synthetic-Tiny10")
    ...
    for index in tqdm(range(13700, 13800)): # SHOULD BE CHANGED BASED ON RANGE!!! If you want to run the whole thing, change it back to range(total)
        arch_str = nets[index]
        hp2info = OrderedDict()
        ...
        for hp in hps:
            ...
            print(f"DEBUG: {arch_info}\n") if args.v else None
            hp2info[hp] = arch_info
        # Examine the hp2info dict
        for hp in hps:
            if hp not in hp2info:
                if args.v:
                    print("HP {:} not in hp2info.".format(hp))
                continue
        ...
        if "12" in hp2info:
            to_save_data["12"] = hp2info["12"].state_dict()
        if "200" in hp2info:
            if args.v:
                print("Saving {:} arch {:}.".format(hp, index)) 
            to_save_data["200"] = hp2info["200"].state_dict()
        ...
        for hp in hps:
            try:
                ...
                print("DEBUG: Cannot clear params for hp={:} arch {:}".format(hp, index)) if args.v else None
                continue
        to_save_data = OrderedDict()
        if "12" in hp2info:
            to_save_data["12"] = hp2info["12"].state_dict()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip_calibration", action="store_true", help="Skip latency calibration."
    )
    parser.add_argument(
        "--dataset", type=str, default=None, help="The dataset to be used."
    )
    parser.add_argument(
