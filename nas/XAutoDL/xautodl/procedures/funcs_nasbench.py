...
# from xautodl.datasets.get_dataset_with_transform import SyntheticDataset, calculate_mean_std
...
def get_nas_bench_loaders(workers, dataset=None):
    if dataset is not None:
        print(f"Using dataset: {dataset}")
    else:
        print("No dataset specified, using all datasets")
    ...
    if dataset is None or dataset == "synthetic-cifar10":
        # Synthetic CIFAR-10
        SYNTH_CIFAR_CONFIG = load_config(
            root_dir / "configs" / "nas-benchmark" / "SYNTH_CIFAR.config", None, None
        )
        cifar_config = load_config(
            root_dir / "configs" / "nas-benchmark" / "CIFAR.config", None, None
        )
        TRAIN_SYNTH_CIFAR10, TEST_SYNTH_CIFAR10, xshape, class_num = get_datasets(
            "synthetic-cifar10", "./data/synthetic-cifar10", -1
        )

        train_synth_cifar10_loader = torch.utils.data.DataLoader(
            TRAIN_SYNTH_CIFAR10,
            batch_size=SYNTH_CIFAR_CONFIG.batch_size,
            shuffle=True,
            num_workers=workers,
            pin_memory=True,
        )
        test_synth_cifar10_loader = torch.utils.data.DataLoader(
            TEST_SYNTH_CIFAR10,
            batch_size=cifar_config.batch_size,
            shuffle=False,
            num_workers=workers,
        ...
        print(
            "Synthetic CIFAR-10 : {:} training images and {:} test images : {:} input shape : {:} number of classes".format(
                len(TRAIN_SYNTH_CIFAR10),
                len(TEST_SYNTH_CIFAR10),
                xshape,
                class_num,
            )
        )
        print(
            "Synthetic CIFAR-10  : train-loader has {:3d} batch with {:} per batch".format(
                len(train_synth_cifar10_loader), SYNTH_CIFAR_CONFIG.batch_size
            )
        )
        print(
            "Synthetic CIFAR-10  : test--loader has {:3d} batch with {:} per batch".format(
                len(test_synth_cifar10_loader), cifar_config.batch_size
            )
        )
        print(break_line)

        loaders = {
            "synthetic-cifar10@train": train_synth_cifar10_loader,
            "synthetic-cifar10@test": test_synth_cifar10_loader,
        }

    if dataset is None or dataset == "Tiny10":
        # Tiny10
        TINY10_CONFIG = load_config(
            root_dir / "configs" / "nas-benchmark" / "TINY10.config", None, None
        )
        TRAIN_TINY10, VALID_TINY10, xshape, class_num = get_datasets(
            "Tiny10", "./data/tiny10", -1
        )
        train_tiny10_loader = torch.utils.data.DataLoader(
            TRAIN_TINY10,
            batch_size=TINY10_CONFIG.batch_size,
            shuffle=True,
            num_workers=workers,
            pin_memory=True,
        )
        valid_tiny10_loader = torch.utils.data.DataLoader(
            VALID_TINY10,
            batch_size=64,
            shuffle=False,
            num_workers=workers,
        ...
        print(
            "original Tiny10 : {:} training images and {:} test images : {:} input shape : {:} number of classes".format(
                len(TRAIN_TINY10), len(VALID_TINY10), xshape, class_num
            )
        )
        print(
            "Tiny10  : train-loader has {:3d} batch with {:} per batch".format(
                len(train_tiny10_loader), TINY10_CONFIG.batch_size
            )
        )
        print(
            "Tiny10  : valid-loader has {:3d} batch with {:} per batch".format(
                len(valid_tiny10_loader), TINY10_CONFIG.batch_size
            )
        )
        print(break_line)
        loaders = {
            "Tiny10@train": train_tiny10_loader,
            "Tiny10@valid": valid_tiny10_loader,
        }

    if dataset is None or dataset == "synthetic-Tiny10":
        # Synthetic Tiny10
        SYNTH_TINY10_CONFIG = load_config(
            root_dir / "configs" / "nas-benchmark" / "SYNTH_TINY10.config", None, None
        )
        TRAIN_SYNTH_TINY10, TEST_SYNTH_TINY10, xshape, class_num = get_datasets(
            "synthetic-Tiny10", "./data/synthetic-Tiny10", -1
        )
        train_synth_tiny10_loader = torch.utils.data.DataLoader(
            TRAIN_SYNTH_TINY10,
            batch_size=SYNTH_TINY10_CONFIG.batch_size,
            shuffle=True,
            num_workers=workers,
            pin_memory=True,
        )
        test_synth_tiny10_loader = torch.utils.data.DataLoader(
            TEST_SYNTH_TINY10,
            batch_size=SYNTH_TINY10_CONFIG.batch_size,
            shuffle=False,
            num_workers=workers,
        ...
        print(
            "Synthetic Tiny10 : {:} training images and {:} test images : {:} input shape : {:} number of classes".format(
                len(TRAIN_SYNTH_TINY10),
                len(TEST_SYNTH_TINY10),
                xshape,
                class_num,
            )
        )
        print(
            "Synthetic Tiny10  : train-loader has {:3d} batch with {:} per batch".format(
                len(train_synth_tiny10_loader), SYNTH_TINY10_CONFIG.batch_size
            )
        )
        print(
            "Synthetic Tiny10  : test--loader has {:3d} batch with {:} per batch".format(
                len(test_synth_tiny10_loader), SYNTH_TINY10_CONFIG.batch_size
            )
        )
        print(break_line)
        loaders = {
            "synthetic-Tiny10@train": train_synth_tiny10_loader,
            "synthetic-Tiny10@test": test_synth_tiny10_loader,
        }

    if dataset is None or dataset == "cifar100":
        # CIFAR-100
    ...
    if dataset is None or dataset == "ImageNet16-120":
        # ImageNet16-120
